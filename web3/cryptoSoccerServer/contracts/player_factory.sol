...
contract PlayerFactory is HelperFunctions {
    ...
    event PlayerCreation(string playerName, uint playerIdx);
    ...
    ///         5-pass
    // @param role is a unit8, limited between 0 and 7, where 0=keeper, 1=defence, 2=midfielder, 3=attacker.
    struct Player {
        string name;
        uint state; 
        uint8 role;
    ...
    function createPlayer(string _playerName, uint _teamIdx, uint8 _playerNumberInTeam, uint _playerState, uint8 _playerRole) 
        internal 
    {
        ...
        players.push(Player({name: _playerName, state: _playerState, role: _playerRole}));
        ...
        emit PlayerCreation(_playerName, nCreatedPlayers);
    }
        ...
        public 
    {
        ...
        string storage teamName = teams[_teamIdx].name;
        uint dna = uint(keccak256(abi.encodePacked(teamName, _userChoice, _playerNumberInTeam)));
        createPlayer(_playerName, _teamIdx, _playerNumberInTeam, computePlayerStateFromRandom(dna, now), _playerRole);
     }

    function computePlayerStateFromRandom(uint longRnd, uint currentTime) internal pure returns(uint) {
        // get random numbers between 0 and 9999:
        uint16[] memory rnds = readNumbersFromUint(6, longRnd, 10000);
        ...
        rnds[0] = uint16((currentTime - rnds[0] * years2secs) / month2secs);
        ...
        excess /= 5;
        for (sk = 1; sk < 6; sk++) {
            rnds[sk] = rnds[sk] + excess;
        }
        return encodeIntoLongIntArray(6, rnds, 10000);
    }
