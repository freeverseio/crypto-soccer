...
with open('../truffle-core/build/contracts/Market.json', 'r') as fp:
    contract = json.load(fp)
with open('./Market.abi', 'w', encoding='utf-8') as outfile:
    json.dump(contract['abi'], outfile, ensure_ascii=False, indent=2)
with open('./Market.bin', 'w', encoding='utf-8') as outfile:
    outfile.write(contract['bytecode'])
os.system('mkdir -p ../go-synchronizer/contracts/market')
os.system('abigen --abi ./Market.abi --bin ./Market.bin --pkg market -out ../go-synchronizer/contracts/market/market.go')
os.system('mkdir -p ../market/notary/contracts/market')
os.system('abigen --abi ./Market.abi --bin ./Market.bin --pkg market -out ../market/notary/contracts/market/market.go')
