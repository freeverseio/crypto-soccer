...
contract('EncodingMatchLog', (accounts) => {
    ...
    it('encode and decode matchlog', async () =>  {
        ...
        typesOutOfGames = [1, 2];
        yellowCards = [9, 6, 3, 0];
        result = await encoding.encodeMatchLog(nGoals, assistersIdx, shootersIdx, penalties, outOfGames, typesOutOfGames, yellowCards);
        result = await encoding.decodeMatchLog(result);
        let {0: nGo, 1: ass, 2: sho, 3: pen, 4: out, 5: typ, 6: yel} = result;
