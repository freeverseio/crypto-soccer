...
contract('Verifier', (accounts) => {
    ...
    beforeEach(async () => {
        verifierLib = await Verifier.new().should.be.fulfilled;
    ...
    it('completes a seller - buyer agreement via MTXs and checks that the BC accepts it', async() => {
        ...
        const sigs = [sigSeller.messageHash, sigSeller.r, sigSeller.s, sigBuyer.messageHash, sigBuyer.r, sigBuyer.s];
    ...
    it('adress type: test abi encode and hash web3 vs solidity', async() => {
        ...
        hashPacked = await abiTestingLib.getHashOfAddress(accounts[0]).should.be.fulfilled
        hashPacked.should.be.equal(
            concatHash([type], [val])
        )
    });
    ...
    it('concat: test abi encode of two args, web3 vs solidity', async() => {
        ...
        abiFromSolidity.should.be.equal(web3.eth.abi.encodeParameters([type1, type2], [val1, val2]))
    });
    ...
    it('signs a hex-like txt message', async() => {
        ...
        console.log('...and sign with either web3.eth.accounts.sign(msg, privKey) or simply account.sign(msg)')
        ...
        console.log('...so that sig1 retains the original data: sig1.message = ' + sig1.message);
        sig1.message.should.be.equal(msg);
        console.log('Most importantly, the msgHashes can be obtained by prepending Ethereum Signed... and then hashing');
        sig1.messageHash.should.be.equal(getMessageHash(sig1.message))

        // checking signatures now:
        let isSigned = await verifierLib.isSigned(accounts[0], sig2.messageHash, sig2.v, sig2.r, sig2.s).should.be.fulfilled;
        isSigned.should.be.equal(false);
        isSigned = await verifierLib.isSigned(newAccount.address, sig1.messageHash, sig1.v, sig1.r, sig1.s).should.be.fulfilled;
        isSigned.should.be.equal(true);
        isSigned = await verifierLib.isSigned(newAccount.address, sig2.messageHash, sig2.v, sig2.r, sig2.s).should.be.fulfilled;
        isSigned.should.be.equal(true);
