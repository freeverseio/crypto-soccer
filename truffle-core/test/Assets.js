...
contract('Assets', (accounts) => {
    ...
    const it1 = async(text, f) => {it2(text, f)};
   ...
   it2('computed skills with rnd = 0 for non goal keepers should be 1000 each', async () => {
    ...
    it('potential adds to a fix average', async () => {
        sumPot = 0;
        for (shirtNum = 0; shirtNum < PLAYERS_PER_TEAM_INIT; shirtNum++) {
            ...
            console.log(shirtNum, pot.toNumber());            
            sumPot += pot.toNumber();
        }
        console.log(sumPot/PLAYERS_PER_TEAM_INIT);
    });
