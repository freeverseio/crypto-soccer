...
contract('Engine', (accounts) => {
    ...
    it('check that penalties are played in playoff games', async () => {
        ...
        log12 = await engine.playMatch(drawSeed = 200, [teamStateAll50, teamStateAll50], [tactics442, tactics1], log = 0, [is2nd = true, isHomeStadium,  playoff = false]).should.be.fulfilled;
        // check that the game would end 3-3
        go12 = await engine.getGoalsFromLog(log12).should.be.fulfilled;
        for (i = 0; i < 2; i++) {
            go12[i].toNumber().should.be.equal(3);
        }
        ...
        log12 = await engine.playMatch(drawSeed, [teamStateAll50, teamStateAll50], [tactics442, tactics1], log, [is2nd = true, isHomeStadium,  playoff = true]).should.be.fulfilled;
        pens = await engine.getPenaltiesFromLog(log12).should.be.fulfilled;
...
return;
    ...
    it('different seeds => different result', async () => {
        ...
        result[0].toNumber().should.be.equal(2);
        result[1].toNumber().should.be.equal(1);
        result = await engine.playMatch(654321, [teamStateAll50, teamStateAll50], [tactics0, tactics1], firstHalfLog, [is2ndHalf, isHomeStadium, isPlayoff]).should.be.fulfilled;
        // console.log(result[0].toNumber(), result[1].toNumber())
        result = await engine.getGoalsFromLog(result).should.be.fulfilled;
