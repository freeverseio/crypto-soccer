...
contract('Engine', (accounts) => {
    ...
    it('select shooter and manages to score', async () => {
        ...
        result = await engine.selectShooter(teamState, playersPerZone442, lineupConsecutive, fwdModifiersNull, kMaxRndNumHalf).should.be.fulfilled;
        result.toNumber().should.be.equal(10);
        result = await engine.managesToScore(teamState, playersPerZone442, lineupConsecutive, fwdModifiersNull, blockShoot = 1, kMaxRndNumHalf, kMaxRndNumHalf).should.be.fulfilled;
        result.should.be.equal(true);
        result = await engine.managesToScore(teamState, playersPerZone442, lineupConsecutive, fwdModifiersNull, blockShoot = 1000, kMaxRndNumHalf, kMaxRndNumHalf).should.be.fulfilled;
        result.should.be.equal(false);
        // even with a super-goalkeeper, there are chances of scoring (e.g. if the rnd is super small, in this case)
        kMaxRndNumHalf = 1;
        result = await engine.managesToScore(teamState, playersPerZone442, lineupConsecutive, fwdModifiersNull, blockShoot = 1000, 8000, 1).should.be.fulfilled;
        result.should.be.equal(true);
    });
