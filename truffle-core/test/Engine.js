...
contract('Engine', (accounts) => {
    ...
    it('wasPlayerAlignedEndOfLastHalf', async () => {
        ...
        log2 = await engine.playHalfMatch(seed, now, [teamStateAll50Half1, teamStateAll50Half1], [tactics, tactics], [0, 0], [is2nd = false, isHomeStadium,  playoff = false]).should.be.fulfilled;
        expected = Array.from(new Array(14), (x,i) => true);
        for (s = 0; s < 3; s++) expected[substis] = false;
        for (p = 0; p < 14; p++) {
            result = await engine.wasPlayerAlignedEndOfLastHalf(0, tactics, log2[0]).should.be.fulfilled;
