...
contract('Engine', (accounts) => {
    ...
    it('computeExceptionalEvents no clashes with redcards', async () => {
        ...
        await logUtils.checkExpectedLog(encodingLog, newLog, nGoals = UNDEF, ass = UNDEF, sho = UNDEF, fwdPos = UNDEF, penalties = UNDEF,
            ...
            halfTimeSubstitutions = UNDEF, nDefs1 = UNDEF, nDefs2 = UNDEF, nTot = UNDEF, winner = UNDEF, teamSumSkills = UNDEF);
    });
    ...
    it('check that nDefs is reduced by one when a defender misses in the 2nd half', async () => {
        ...
        for (team = 0; team < 2; team++){
            ...
            nDefs.toNumber().should.be.equal(0); // 0 because we did not playMatch in 1st half
    ...
    it('teamSkills are added from 1st to 2nd half', async () => {
        ...
        log0 =  await engine.playMatch(seedDraw,  now, [teamStateAll50Half1, teamStateAll50Half1], [tactics442TwoChanges, tactics442WithNoChanges], log = [0, 0], [is2nd = false, isHomeStadium, isPlayoff]).should.be.fulfilled;
        expected = [3250, 2750];
        for (team = 0; team < 2; team++) {
        ...
        log12 = await engine.playMatch(seedDraw,  now, [teamStateAll50Half2, teamStateAll50Half2], [tactics442OneChange, tactics442WithNoChanges], log0, [is2nd = true, isHomeStadium, isPlayoff]).should.be.fulfilled;
        expected = [3322, 2750];
        for (team = 0; team < 2; team++) {
