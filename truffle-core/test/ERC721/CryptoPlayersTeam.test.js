...
contract('PlayersTeam', (accounts) => {
    let contract = null;
    const teamsContract = accounts[1];

    beforeEach(async () => {
        contract = await Players.new().should.be.fulfilled;
        await contract.addTeamsContract(teamsContract).should.be.fulfilled;
        await contract.renounceTeamsContract().should.be.fulfilled;
    });
    ...
    it('coach can set the team', async () => {
        ...
        await contract.mint(accounts[0], "player").should.be.fulfilled;
        const playerId = await contract.getPlayerId("player").should.be.fulfilled;
        await contract.setTeam(playerId, teamId, {from: teamsContract}).should.be.fulfilled;
    });
