...
contract('Updates', (accounts) => {
    ...
    it('true status of timezone challenge', async () =>  {
        ...
        level.toNumber().should.be.equal(0);
        isSet.should.be.equal(true);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(3.5*challengeTime), lastUpdate = 0, writtenLevel = 2).should.be.fulfilled;
        level.toNumber().should.be.equal(0);
        isSet.should.be.equal(true);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(0.5*challengeTime), lastUpdate = 0, writtenLevel = 3).should.be.fulfilled;
        level.toNumber().should.be.equal(3);
        isSet.should.be.equal(false);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(1.5*challengeTime), lastUpdate = 0, writtenLevel = 3).should.be.fulfilled;
        level.toNumber().should.be.equal(1);
        isSet.should.be.equal(false);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(2.5*challengeTime), lastUpdate = 0, writtenLevel = 3).should.be.fulfilled;
        level.toNumber().should.be.equal(1);
        isSet.should.be.equal(true);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(3.5*challengeTime), lastUpdate = 0, writtenLevel = 3).should.be.fulfilled;
        level.toNumber().should.be.equal(1);
        isSet.should.be.equal(true);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(0.5*challengeTime), lastUpdate = 0, writtenLevel = 4).should.be.fulfilled;
        level.toNumber().should.be.equal(4);
        isSet.should.be.equal(false);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(1.5*challengeTime), lastUpdate = 0, writtenLevel = 4).should.be.fulfilled;
        level.toNumber().should.be.equal(2);
        isSet.should.be.equal(false);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(2.5*challengeTime), lastUpdate = 0, writtenLevel = 4).should.be.fulfilled;
        level.toNumber().should.be.equal(0);
        isSet.should.be.equal(false);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(3.5*challengeTime), lastUpdate = 0, writtenLevel = 4).should.be.fulfilled;
        level.toNumber().should.be.equal(0);
        isSet.should.be.equal(true);
        var {0: level, 1: isSet} = await updates.getStatusPure(nowTime = Math.floor(4.5*challengeTime), lastUpdate = 0, writtenLevel = 4).should.be.fulfilled;
        level.toNumber().should.be.equal(0);
        isSet.should.be.equal(true);
    });
