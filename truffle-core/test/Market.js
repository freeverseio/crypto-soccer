...
async function signPutForSaleMTx(currencyId, price, rnd, validUntil, playerId, sellerAccount) {
  ...
  const sellerTxMsg = concatHash(
      ...
      [hiddenPrice, validUntil, playerId]
  )
...
function buildOfferToBuyMsg(currencyId, price, rnd, validUntil, playerId, buyerTeamId) {
...
async function signOfferToBuyMTx(currencyId, price, rnd, validUntil, playerId, buyerTeamId, buyerAccount) {
  const buyerTxMsg = buildOfferToBuyMsg(currencyId, price, rnd, validUntil, playerId, buyerTeamId);
  const sigBuyer = await buyerAccount.sign(buyerTxMsg);
...
// Buyer explicitly agrees to all of sellers data, and only adds the 'buyerTeamId' to it.
async function signAgreeToSellMTx(currencyId, price, rnd, validUntil, playerId, buyerTeamId, sellerAccount) {
  const sellerTxMsg = buildOfferToBuyMsg(currencyId, price, rnd, validUntil, playerId, buyerTeamId);
  const sigSeller = await sellerAccount.sign(sellerTxMsg);
  return sigSeller;
}
...
contract("Market", accounts => {
  ...
  it('deterministic sign (values used in market.notary test)', async () => {
    ...
    const buyerMsg = await market.buildAgreeToBuyTxMsg(prefixed, buyerHiddenPrice, buyerTeamId, isOffer2StartAuction).should.be.fulfilled;
