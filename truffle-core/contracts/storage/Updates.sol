...
contract Updates is UpdatesBase {
    ...
    function initUpdates() external onlyCOO {
        ...
        uint256 secsOfDay   = PRODUCTION_DEPLOY_TIME % (3600 * 24);
        ...
        if (minute < 27) {
            ...
            _nextVerseTimestamp = PRODUCTION_DEPLOY_TIME + (29-minute)*60 + (60 - secs);
        } else {
            _timeZoneForRound1 = normalizeTZ(1+uint8(hour));
            _nextVerseTimestamp = PRODUCTION_DEPLOY_TIME + (29-minute)*60 + (60 - secs) + 3600;
        }
