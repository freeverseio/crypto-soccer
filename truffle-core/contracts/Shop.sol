...
contract Shop is EncodingSkillsSetters{
    ...
    struct ShopItem {
        ...
        bytes32 ownerTeamsHash;
        bytes32 signedTeamsHash;
        uint16 initStock;
    ...
    function getOwnerTeamsHash(uint256 itemId) public view returns (bytes32) { return _shopItems[itemId].ownerTeamsHash; }
    function getSignedTeamsHash(uint256 itemId) public view returns (bytes32) { return _shopItems[itemId].signedTeamsHash; }
    ...
    function offerItem(uint256 skillsBoost, uint16 initStock, uint8 matchesDuration, string memory uri, uint64[] memory leagueIds) public {
        ...
        _shopItems.push(ShopItem(skillsBoost, merkleRootTemp(leagueIds), 0, 0, initStock, matchesDuration, uri));
