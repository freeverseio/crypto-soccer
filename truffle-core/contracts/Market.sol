...
contract Market is Assets {
    ...
    function freezePlayer(
        ...
        require(getPlayerOwner(playerId) != NULL_ADDR, "player not owned by anyone");
        require(getOwnerTeam(teamId) != NULL_ADDR, "team not owned by anyone");
        ...
        require(getOwnerTeam(teamId) == recoverAddr(sigs[BUY_MSG], vs[BUYER], sigs[BUY_r], sigs[BUY_s]),
    ...
    function getPlayerOwner(uint256 playerId) public view returns (address) {
        ...
        uint256 teamId = _assetsLib.getCurrentTeamId(state);
