...
contract Engine is EngineLib, EncodingMatchLogPart3 {
    ...
    uint8 public constant MAX_GOALS_IN_HALF  = 7;   // Max number of goals that one single team can score in one given half (in a match, it's 2x this val)
        ...
        returns (uint256[4] memory scoreData)
    {
        ...
        if (currentGoals >= MAX_GOALS_IN_HALF) return scoreData;
