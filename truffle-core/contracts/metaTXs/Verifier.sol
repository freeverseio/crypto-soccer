...
contract Verifier {
    ...
    mapping (uint256 => address) private teamOwner;
    mapping (uint256 => address) private playerOwner;
    ...
    function freezePlayer(
        ...
        require(playerOwner[playerId] != address(0), "player not owned by anyone");
        require(teamOwner[playerId] != address(0), "team not owned by anyone");

        // check signatures are valid by requiring that they own the asset:
        require(playerOwner[playerId] == recoverAddr(sigs[SELL_MSG], vs[SELLER], sigs[SELL_r], sigs[SELL_s]),
            "seller is not owner of player, or seller signature is not valid");
        require(teamOwner[playerId]   == recoverAddr(sigs[BUY_MSG], vs[BUYER], sigs[BUY_r], sigs[BUY_s]),
    ...
    // Management that fakes the data structs that there would be in the game
    function setTeamOwner(uint256 playerId, address ownerAddr) external {
        teamOwner[playerId] = ownerAddr;
    }

    function setPlayerOwner(uint256 playerId, address ownerAddr) external {
        playerOwner[playerId] = ownerAddr;
    }
