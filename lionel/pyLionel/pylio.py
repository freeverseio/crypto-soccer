...
def exchangePlayers(playerIdx1, address1, playerIdx2, address2, currentBlock, ST):
    assert not isPlayerBusy(playerIdx1, currentBlock, ST), "Player sale failed: player is busy playing a league, wait until it finishes"
    assert not isPlayerBusy(playerIdx2, currentBlock, ST), "Player sale failed: player is busy playing a league, wait until it finishes"
    ...
    state1.setLastSaleBlocknum(currentBlock)
    state2.setLastSaleBlocknum(currentBlock)
...
def isPlayerBusy(playerIdx1, blocknumber, ST):
...
def areTeamsBusyInPrevLeagues(teamIdxs, blocknumber, ST):
    for teamIdx in teamIdxs:
        if not ST.leagues[ST.teams[teamIdx].currentLeagueIdx].isFullyVerified(blocknumber):
            return True
    return False
...
def createLeague(currentBlock, blocknumber, blockStep, usersInitData, ST):
    assert not areTeamsBusyInPrevLeagues(usersInitData["teamIdxs"], currentBlock, ST), "League cannot create: some teams involved in prev leagues"
...
def createLeagueClient(currentBlock, blocknumber, blockStep, usersInitData, ST_CLIENT):
    assert not areTeamsBusyInPrevLeagues(usersInitData["teamIdxs"], currentBlock, ST_CLIENT), "League cannot create: some teams involved in prev leagues"
