...
def test1():
    ...
    blockCounter.advanceNBlocks(10)
    exchangePlayers(
        2, ADDR1,
        24, ADDR2,
...
def test2():
    ...
    ST_CLIENT   = Storage()
    blockCounter = BlockCounter()
    blockCounter.advanceToBlock(10)
    ...
    blockCounter.advanceToBlock(100)
    blockInit = 190
    blockStep = 10
    ...
    leagueIdx          = createLeague(blockCounter.currentBlock, blockInit, blockStep, usersInitData, ST)
    leagueIdx_client   = createLeagueClient(blockCounter.currentBlock, blockInit, blockStep, usersInitData, ST_CLIENT)

    assert ST.leagues[leagueIdx].isLeagueIsAboutToStart(blockCounter.currentBlock), "League not detected as created"

    # Advance to matchday 2
    blockCounter.advanceToBlock(blockInit + blockStep - 5)
    assert ST.leagues[leagueIdx].hasLeagueStarted(blockCounter.currentBlock), "League not detected as already being played"
    assert not ST.leagues[leagueIdx].hasLeagueFinished(blockCounter.currentBlock), "League not detected as not finished yet"
    ...
    usersAlongData = {
        ...
        "block": blockCounter.currentBlock
    }
    ...
    blockCounter.advanceNBlocks(blockStep)
    assert ST.leagues[leagueIdx].hasLeagueFinished(blockCounter.currentBlock), "League not detected as already finished"
    ...
    blockCounter.advanceNBlocks(CHALLENGING_PERIOD_BLKS-5)
    assert not ST.leagues[leagueIdx].isFullyVerified(blockCounter.currentBlock)
    selectedTeam = 0
    ...
    blockCounter.advanceNBlocks(CHALLENGING_PERIOD_BLKS-5)
    assert not ST.leagues[leagueIdx].isFullyVerified(blockCounter.currentBlock), "League not detected as not-yet fully verified"
    ...
    blockCounter.advanceNBlocks(2)
    assert not ST.leagues[leagueIdx].isFullyVerified(blockCounter.currentBlock), "League not detected as not-yet fully verified"
    blockInit = blockCounter.currentBlock + 30
    usersInitData = {
    ...
    try:
        leagueIdx2 = createLeague(blockCounter.currentBlock, blockInit, blockStep, usersInitData, ST)
        itFailed = False
    except:
        itFailed = True
    ...
    blockCounter.advanceNBlocks(CHALLENGING_PERIOD_BLKS-5)
    assert ST.leagues[leagueIdx].isFullyVerified(blockCounter.currentBlock), "League not detected as already fully verified"
    ...
    leagueIdx2          = createLeague(blockCounter.currentBlock, blockInit, blockStep, usersInitData, ST)
    leagueIdx2_client   = createLeagueClient(blockCounter.currentBlock, blockInit, blockStep, usersInitData, ST_CLIENT)

    assert leagueIdx2 == leagueIdx2_client, "Leagues in client not in sync with BC"

    # An UPDATER updates:
    blockCounter.advanceNBlocks(1000)
    assert ST.leagues[leagueIdx].hasLeagueFinished(blockCounter.currentBlock), "League should be finished by now"
...
success = success and runTest(name = "Test Entire Workflow",      result = test2(), expected = 540)
if success:
