...
import (
	...
	"github.com/freeverseio/crypto-soccer/go/contracts/proxy2contracts"
...
func New(
	...
	contracts.Assets, err = proxy2contracts.NewAssets(common.HexToAddress(assetsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("failed to connect to the ethereum client: %v", err)
	}
	...
	contracts.Utils, err = proxy2contracts.NewUtils(common.HexToAddress(utilsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("Failed to connect to the Ethereum client: %v", err)
	}
