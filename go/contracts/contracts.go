...
type Contracts struct {
	...
	Shop                    *shop.Shop
	TrainingPoints          *router.Trainingpoints
...
func New(
	...
	contracts.Assets, err = router.NewAssets(common.HexToAddress(assetsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("failed to connect to the ethereum client: %v", err)
	}
	...
	contracts.Utils, err = router.NewUtils(common.HexToAddress(utilsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("Failed to connect to the Ethereum client: %v", err)
	}
	...
	if contracts.TrainingPoints, err = router.NewTrainingpoints(common.HexToAddress(trainingpointsAddress), contracts.Client); err != nil {
		return nil, err
	}
