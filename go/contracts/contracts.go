...
import (
	...
	"github.com/freeverseio/crypto-soccer/go/contracts/native"
...
func New(
	...
	contracts.Assets, err = native.NewAssets(common.HexToAddress(assetsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("failed to connect to the ethereum client: %v", err)
	}
	...
	contracts.Utils, err = native.NewUtils(common.HexToAddress(utilsAddress), contracts.Client)
	if err != nil {
		log.Fatalf("Failed to connect to the Ethereum client: %v", err)
	}
