...
func (b *Calendar) ResetBulk(tx *sql.Tx, timezoneIdx uint8, countryIdx uint32, leagueIdx uint32) error {
	league, err := storage.LeagueByLeagueIdx(tx, leagueIdx)
	if err != nil {
		return err
	}
...
func (b *Calendar) PopulateBulk(tx *sql.Tx, timezoneIdx uint8, countryIdx uint32, leagueIdx uint32, matchesStart [14]*big.Int, teamsInLeague []storage.Team) error {
	league, err := storage.LeagueByLeagueIdx(tx, leagueIdx)
	if err != nil {
		return err
	}
...
func (b *Calendar) PopulateByTzCountry(tx *sql.Tx, timezoneIdx uint8, countryIdx uint32, matchesStart [14]*big.Int, leagues []storage.League) error {
	var matchesToSetTeams []storage.Match

	for _, league := range leagues {
		teamsInLeague, err := storage.TeamsByTimezoneIdxCountryIdxLeagueIdxOrdered(tx, timezoneIdx, countryIdx, league.LeagueIdx)
		if err != nil {
			return err
		}
		for matchDay := uint8(0); matchDay < contracts.MatchDays; matchDay++ {
