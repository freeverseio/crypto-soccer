...
type EventProcessor struct {
	...
	directoryContractAddress string
...
func (b EventProcessor) getFirstDeployEvent(directoryAddress string) (*directory.DirectoryDeployedDirectory, error) {
	directoryContract, err := directory.NewDirectory(common.HexToAddress(directoryAddress), b.Client)
	if err != nil {
		return nil, err
	}
...
func (p *EventProcessor) Process(tx *sql.Tx, delta uint64) (uint64, error) {
	...
	if lastProcessedBlock == 0 {
		...
		bigBangEvent, err := p.getFirstDeployEvent(p.directoryContractAddress)
		if err != nil {
			return 0, err
		}
		if bigBangEvent == nil {
		...
		c, err := contracts.NewByDeployedDirectoryEvent(p.Client, *bigBangEvent)
		if err != nil {
			return 0, err
		}
		if p.staker != nil {
		...
		if err := postgres.ToStorage(tx, c); err != nil {
			return 0, err
		}
		p.contracts = c
