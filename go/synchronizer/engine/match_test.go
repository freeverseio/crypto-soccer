...
func TestFromTheField(t *testing.T) {
	...
	t.Log(match.Seed)
	t.Log(match.HomeTeam.TeamID)
	t.Log(match.VisitorTeam.TeamID)
	t.Log(match.HomeTeam.PlayerIDs())
	t.Log(match.VisitorTeam.PlayerIDs())
	t.Log(homeTactic)
	t.Log(visitorTactic)
	t.Log(decodedHomeMatchLog)
	t.Log(decodedVisitorMatchLog)
	t.Log([]*big.Int{homeMatchLog, visitorMatchLog})
