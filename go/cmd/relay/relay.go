...
func main() {
	...
	xoloClient := flag.String("xolo", "http://localhost:8545", "xolo node")
	...
	log.Info("Dial the Xolo client: ", *xoloClient)
	xbackend, err := xolo.NewAbiBackend(
		xolo.NewGethClientFactory(),
		[]string{*ethereumClient},
		[]string{*xoloClient})
	if err != nil {
		log.Fatal(err)
	}
	...
	updatesContract, err := updates.NewUpdates(common.HexToAddress(*updatesContractAddress), xbackend)
	if err != nil {
	...
	processor, err := relay.NewProcessor(xbackend.TransactOps(), db, updatesContract, *ipfsURL)
	if err != nil {
		log.Fatal(err)
	}
