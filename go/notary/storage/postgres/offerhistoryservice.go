...
func (b StorageHistoryService) OfferInsert(tx *sql.Tx, offer storage.Offer) (int64, error) {
	id, err := b.StorageService.OfferInsert(tx, offer)
	if err != nil {
		return 0, err
	}
	offer.ID = id
	return id, offerInsertHistory(tx, offer)
}
