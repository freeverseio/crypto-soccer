...
func TestCreateAuctionCallRollbackOnError(t *testing.T) {
	...
	mock := mockup.Tx{
		CancelAllOffersByPlayerIdFunc: func(id string) error { return nil },
...
func TestCreateAuctionCallRollbackOnErrorCancelingOffers(t *testing.T) {
	timezoneIdx := uint8(1)
	countryIdx := big.NewInt(0)
	// We will here assign the next available team to offerer so she can make an offer for the players of a different team
	// we choose that player as a player very far from the current amount of teams (x2)
	...
	mock := mockup.Tx{
		CancelAllOffersByPlayerIdFunc: func(id string) error { return errors.New("errorCancelAllOffersByPlayerIdFunc") },
	...
	assert.Error(t, err, "errorCancelAllOffersByPlayerIdFunc")
	assert.Equal(t, rollbackCounter, 1)
}
