...
func TestGetWorldPlayersDeterministicResult(t *testing.T) {
	...
	assert.Equal(t, len(players0), 44)
...
func TestGetWorldPlayersOfSoldPlayer(t *testing.T) {
	...
	players, err = service.CreateBatch(teamId, now)
	assert.NilError(t, err)
	assert.Equal(t, len(players), 44-1)
}
...
func TestWorldPlayerServiceTimeDependentDiscontinuity(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.Equal(t, len(players), 44)
	assert.Equal(t, string(players[0].PlayerId()), "25726330598748577796374503147506029554480293493442174140809939")
	...
	assert.NilError(t, err)
	assert.Equal(t, len(players), 44)
	assert.Equal(t, string(players[0].PlayerId()), "25726327533757516004320599914406576701205026123188100402250445")
