...
CREATE TABLE training (
    ...
    defences_defence INT CHECK (defences_defence >= 0),
    defences_speed INT CHECK (defences_speed >= 0),
    defences_pass INT CHECK (defences_pass >= 0),
    defences_shoot INT CHECK (defences_shoot >= 0),
    defences_endurance INT CHECK (defences_endurance >= 0),

    middles_defence INT CHECK (middles_defence >= 0),
    middles_speed INT CHECK (middles_speed >= 0),
    middles_pass INT CHECK (middles_pass >= 0),
    middles_shoot INT CHECK (middles_shoot >= 0),
    middles_endurance INT CHECK (middles_endurance >= 0),
